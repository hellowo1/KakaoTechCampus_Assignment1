...
- **할 일 관리 (CRUD)** — 추가 / 수정 / 완료 / 삭제
  - 빈 값으로는 추가되지 않고 안내 메시지를 표시
  - 완료한 할 일은 취소선으로 구분
  - 수정은 `prompt()` 없이 항목 안에서 바로(인라인 입력창, Enter 저장 / Esc 취소)
- **상태 필터** — 전체 / 진행 중 / 완료
...
```js
{
  id: 1717804800000,   // 고유 id (Date.now())
  text: "운동하기",     // 할 일 내용
  isCompleted: false,  // 완료 여부
  date: "2026-06-08"   // 어느 날짜의 할 일인지 (YYYY-MM-DD)
}
```

할 일이 어느 날에 속하는지는 오직 `date` 값으로 결정돼요.
그래서 이월도 항목을 옮기는 게 아니라 `date`만 다음 날로 바꾸면, 화면과 주간 달력 개수가 알아서 갱신돼요.
